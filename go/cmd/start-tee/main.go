...
import (
	...
	extserver "activity-reward-extension/pkg/server"
...
// devPrivateKeyHex is the well-known local-devnet funded key (Hardhat/Anvil
// account 0), published in both projects' own docs. It has funds ONLY on a local
// devnet, so it is not a credential. Kept as a named constant rather than an
// inline literal so there is one auditable occurrence per module (tools/ has
// its own in pkg/configs: the two are separate Go modules and cannot share it
// without coupling the extension image to the tools module). Use is gated on
// LOCAL_MODE=true by setOwnerAddress below, and fails closed otherwise.
...
func setOwnerAddress() common.Address {
	...
	if owner == "" {
		...
		if privKeyString == "" {
			// Fail closed like support.DefaultPrivateKey: the well-known Hardhat
			// dev key must never become the owner outside an explicitly-declared
			// local devnet — anyone could take over a TEE owned by it.
			if !strings.EqualFold(os.Getenv("LOCAL_MODE"), "true") {
				logger.Fatalf("INITIAL_OWNER and DEPLOYMENT_PRIVATE_KEY are both unset — set one, or set LOCAL_MODE=true to use the local-devnet dev key")
			}
			logger.Warnf("INITIAL_OWNER unset — deriving it from the local-devnet dev key (LOCAL_MODE=true)")
			privKey, err = crypto.HexToECDSA(devPrivateKeyHex)
			if err != nil {
