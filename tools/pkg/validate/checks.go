...
// registrationScanBudget bounds the registration checks' chain reads, R7's per-id
// scan included. A validator that hangs reports nothing at all.
const registrationScanBudget = 2 * time.Minute
...
// instructionSenderReader is the one registry method the duplicate scan needs.
// Declared here, where it is consumed, so the scan's failure handling is testable
// without a chain: whether an unreadable id is treated as "did not match" is the
// security property, and it was not verifiable against the concrete generated binding.
type instructionSenderReader interface {
	GetTeeExtensionInstructionsSender(opts *bind.CallOpts, extensionID *big.Int) (common.Address, error)
}

// scanForDuplicateSenders returns every extension id in [0, last) whose registered
// instruction sender is want.
//
// It returns an error rather than a short list when the registry could not be read to
// the end. That distinction is the whole value of the check: a partial scan that finds
// one match looks exactly like a complete scan that found one match, and the second
// means "unique" while the first means "unknown".
func scanForDuplicateSenders(ctx context.Context, reg instructionSenderReader, opts *bind.CallOpts, want common.Address, last int64) ([]string, error) {
	var ids []string
	for i := int64(0); i < last; i++ {
		if err := ctx.Err(); err != nil {
			return nil, fmt.Errorf("the scan ran out of time after reading ids 0..%d of %d: %w", i-1, last-1, err)
		}
		sender, err := reg.GetTeeExtensionInstructionsSender(opts, big.NewInt(i))
		if err != nil {
			return nil, fmt.Errorf("reading the instruction sender of extension %d: %w", i, err)
		}
		if sender == want {
			ids = append(ids, big.NewInt(i).String())
		}
	}
	return ids, nil
}
...
func RegisterRegistrationChecks(
	...
	// Bounded, not context.Background(): the R7 scan below makes one call per
	// registered extension, so an unbounded context lets a slow or hanging RPC stall
	// the validator for as long as the registry is large. The budget is checked
	// inside that loop and running out is reported, never absorbed.
	callCtx, cancelCalls := context.WithTimeout(context.Background(), registrationScanBudget)
	defer cancelCalls()
	callOpts := &bind.CallOpts{Context: callCtx}

	// R1: Check the registry's next public extension id.
	var counter *big.Int
	...
	//
	// This is the check that looks for the pre-registration signature: the contract's
	// CREATE address is deterministic, the registry permits registering it under more
	// than one extension, and a duplicate satisfies the contract's own expectedId
	// check for BOTH ids. So "is this address unique in the registry" is a security
	// question, and answering it requires having read every id — an id that could not
	// be read is not an id that did not match.
	if counter != nil && counter.Sign() > 0 && addressRegex.MatchString(instrSender) && registry != nil && client != nil {
		matchingIDs, scanErr := scanForDuplicateSenders(callCtx, registry, callOpts, common.HexToAddress(instrSender), counter.Int64())
		switch {
		case scanErr != nil:
			// Not a PASS. Reporting "unique" on a partial scan is the failure mode this
			// check exists to prevent, so an incomplete read is stated as one.
			r.Add(CheckResult{
				Step:    "register",
				ID:      "R7",
				Name:    "unique instruction sender",
				Status:  FAIL,
				Message: fmt.Sprintf("could not establish whether %s is registered under more than one extension: %v", instrSender, scanErr),
				Fix:     "Retry against a responsive RPC. Until the whole registry has been read, pass the extension id you registered explicitly rather than letting a tool resolve it.",
			})
		case len(matchingIDs) > 1:
			// FAIL, not WARN: verify-deploy exits non-zero on FAIL, and this is the state
			// that enables the pre-registration attack. ResolveExtensionId already refuses
			// outright in the same situation — a validator that only warns about it
			// contradicts the resolver.
			r.Add(CheckResult{
				Step:    "register",
				ID:      "R7",
				Name:    "unique instruction sender",
				...
				Fix:     "A deterministic CREATE address registered under more than one extension is the pre-registration signature. Confirm which id you registered and pass it explicitly (setExtensionId / -extension-id); do not let a tool choose.",
			})
		default:
			r.Add(CheckResult{
				Step:    "register",
...
func RegisterTeeMachineChecks(r *Report, extensionEnvPath string) {
	...
	if simulatedTee == "true" && localMode == "false" {
		r.Add(CheckResult{
			...
			Status: WARN,
			Message: "SIMULATED_TEE=true on non-local deployment — machine will be registered with test attestation values, " +
				"which is expected for TEE_PROFILE=testnet-sim and wrong for real hardware",
			// Do NOT advise flipping this alone: SIMULATED_TEE must agree with
			// MODE. With MODE=1 the node emits the magic_pass sentinel, and
			// SIMULATED_TEE=false then makes register-tee parse it as a real
			// attestation JWT and abort.
			Fix: "For real TEE hardware switch the whole profile: TEE_PROFILE=confidential-space with MODE=0 and SIMULATED_TEE=false",
		})
