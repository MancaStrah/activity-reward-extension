...
// --- R7: the duplicate-instruction-sender scan ------------------------------------
//
// R7 answers "is this address registered under more than one extension" — the
// pre-registration signature. The answer is only meaningful if every id was read, so
// these cases are mostly about the ways the scan can come up short.

type stubSenderReader struct {
	senders map[int64]common.Address
	errOn   map[int64]bool
	reads   []int64
}

func (s *stubSenderReader) GetTeeExtensionInstructionsSender(_ *bind.CallOpts, id *big.Int) (common.Address, error) {
	s.reads = append(s.reads, id.Int64())
	if s.errOn[id.Int64()] {
		return common.Address{}, errors.New("rpc: rate limit exceeded")
	}
	return s.senders[id.Int64()], nil
}

func TestScanForDuplicateSendersRefusesAPartialRead(t *testing.T) {
	target := common.HexToAddress("0x00000000000000000000000000000000000000AA")
	// The duplicate lives at id 2, and id 0 is the one the RPC refuses. Skipping id 0
	// would leave exactly one match and read as "unique" — the defect this test pins.
	reg := &stubSenderReader{
		senders: map[int64]common.Address{0: target, 1: {}, 2: target},
		errOn:   map[int64]bool{0: true},
	}

	ids, err := scanForDuplicateSenders(context.Background(), reg, &bind.CallOpts{}, target, 3)
	if err == nil {
		t.Fatalf("an unreadable id must not be reported as a completed scan, got %v", ids)
	}
	if ids != nil {
		t.Fatalf("no partial list may be returned alongside the error, got %v", ids)
	}
	if !strings.Contains(err.Error(), "extension 0") {
		t.Errorf("the error should name the id that could not be read: %v", err)
	}
}

func TestScanForDuplicateSendersRefusesAnExhaustedBudget(t *testing.T) {
	target := common.HexToAddress("0x00000000000000000000000000000000000000AA")
	reg := &stubSenderReader{senders: map[int64]common.Address{0: target, 1: target}}

	ctx, cancel := context.WithCancel(context.Background())
	cancel() // already done before the first read

	ids, err := scanForDuplicateSenders(ctx, reg, &bind.CallOpts{}, target, 2)
	if err == nil {
		t.Fatalf("an unfinished scan must not yield a list, got %v", ids)
	}
	if len(reg.reads) != 0 {
...
func TestScanForDuplicateSendersFindsEveryMatch(t *testing.T) {
	target := common.HexToAddress("0x00000000000000000000000000000000000000AA")
	other := common.HexToAddress("0x00000000000000000000000000000000000000BB")
	reg := &stubSenderReader{senders: map[int64]common.Address{0: target, 1: other, 2: target}}

	ids, err := scanForDuplicateSenders(context.Background(), reg, &bind.CallOpts{}, target, 3)
	if err != nil {
		t.Fatalf("a complete scan must not error: %v", err)
	}
	if len(ids) != 2 || ids[0] != "0" || ids[1] != "2" {
		t.Fatalf("both registrations must be reported, got %v", ids)
	}
	// It must not stop at the first match; a scan that did could never find the second.
	if len(reg.reads) != 3 {
		t.Errorf("the whole range must be read, read %v", reg.reads)
	}
}

func TestScanForDuplicateSendersReportsUniqueness(t *testing.T) {
	target := common.HexToAddress("0x00000000000000000000000000000000000000AA")
	other := common.HexToAddress("0x00000000000000000000000000000000000000BB")
	reg := &stubSenderReader{senders: map[int64]common.Address{0: other, 1: target, 2: other}}

	ids, err := scanForDuplicateSenders(context.Background(), reg, &bind.CallOpts{}, target, 3)
	if err != nil || len(ids) != 1 || ids[0] != "1" {
		t.Fatalf("a single match on a complete scan is the PASS case: ids=%v err=%v", ids, err)
	}
}
