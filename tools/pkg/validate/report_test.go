...
// TestFprintEscapesHostileCheckText pins the report writer as the choke point for
// untrusted text.
//
// Check messages are assembled from values this tool does not control — a field out of
// extension.env, an on-chain string, the text of an RPC error. A report is read to
// decide whether a deployment is sound, so a message able to move the cursor or forge
// a line can forge a verdict: the payload below ends a FAIL block and opens what looks
// like a PASS one.
func TestFprintEscapesHostileCheckText(t *testing.T) {
	hostile := "rpc error\r\n  \x1b[32m[PASS]\x1b[0m R7  unique instruction sender"

	var r Report
	r.Add(CheckResult{Step: "register", ID: "R7", Name: "unique instruction sender",
		Status: FAIL, Message: hostile, Fix: hostile})

	var buf bytes.Buffer
	r.Fprint(&buf)
	got := buf.String()

	// NOT "no ESC anywhere": Fprint emits its own colour codes, so the assertion has
	// to be about the payload. (The first version of this test asserted the blanket
	// property and failed on the report's own colouring — worth keeping in mind.)
	if strings.Contains(got, hostile) {
		t.Errorf("the hostile text reached the output verbatim:\n%q", got)
	}
	if strings.Contains(got, "\r") {
		t.Errorf("a live CR reached the output; Fprint emits none of its own:\n%q", got)
	}
	// Escaped, not dropped: a message that carried control bytes should look
	// suspicious rather than merely shorter.
	if n := strings.Count(got, `\x1b[32m`); n != 2 {
		t.Errorf("expected the ESC rendered visibly twice (message + fix), got %d:\n%q", n, got)
	}
	if !strings.Contains(got, "unique instruction sender") {
		t.Errorf("the text itself should survive, escaped:\n%q", got)
	}
	// The payload tried to close the FAIL block and open a PASS one; the only lines
	// in the output must be the ones Fprint wrote.
	if n := strings.Count(got, "[PASS]"); n != 2 {
		t.Errorf("expected the escaped payload twice, got %d:\n%q", n, got)
	}
}
