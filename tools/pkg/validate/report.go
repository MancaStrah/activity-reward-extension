	...
	for _, step := range steps {
		...
		for _, res := range r.Results {
			...
			if res.Status == FAIL || res.Status == WARN {
				// Sanitized here rather than at each construction site, because that is
				// the choke point every check message passes through. Messages are built
				// from values this tool does not control — an extension.env field, an
				// on-chain string, the text of an RPC error — and a report is read to
				// decide whether a deployment is sound, so a message that can move the
				// cursor or forge a line can forge a verdict.
				if res.Message != "" {
					fmt.Fprintf(w, "         %s\n", fccutils.SanitizeForTerminal(res.Message))
				}
				if res.Fix != "" {
					fmt.Fprintf(w, "         Fix: %s\n", fccutils.SanitizeForTerminal(res.Fix))
				}
			}
		}
		fmt.Fprintln(w)
