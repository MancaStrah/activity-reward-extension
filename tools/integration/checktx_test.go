...
func TestCheckTx_SuccessfulTx(t *testing.T) {
	...
	_, tx, _, err := strava.DeployStravaInstructionSender(
...
func TestCheckTx_FailedTx_DecodesReason(t *testing.T) {
	...
	// Call setExtensionId with a public id the registry does not map to this fresh
	// contract — it reverts on the registry-binding check. We bypass gas estimation
	// with a manual gas limit so the tx is submitted and mined (rather than failing
	// at estimation).
	opts, err := bind.NewKeyedTransactorWithChainID(testSupport.Prv, testSupport.ChainID)
	if err != nil {
		t.Fatalf("transactor: %v", err)
	...
	tx, err := contract.SetExtensionId(opts, big.NewInt(0x10000))
	if err != nil {
