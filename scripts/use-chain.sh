...
list_chains() {
    ...
    for f in "$PROJECT_DIR"/.env.*; do
        [[ -f "$f" ]] || continue
        name="${f##*/.env.}"
        case "$name" in example|local|template|sample|*.example) continue ;; esac
        echo "  $name"
        any=1
    done
...
# .env holds private keys — never leave it readable beyond the owner (the
# source .env.<chain> gets the same treatment so the keys are protected at rest).
chmod 600 "$DST" "$SRC" || die "failed to chmod 600 $DST"
