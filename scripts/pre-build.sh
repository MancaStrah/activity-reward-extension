...
INSTRUCTION_SENDER=$(go run ./cmd/deploy-contract -a "$ADDRESSES_FILE" -c "$CHAIN_URL" 2>>"$LOG_FILE" | tee -a "$LOG_FILE" | tail -1) || {
...
EXTENSION_ID=$(go run ./cmd/register-extension -a "$ADDRESSES_FILE" -c "$CHAIN_URL" --instructionSender "$INSTRUCTION_SENDER" 2>>"$LOG_FILE" | tee -a "$LOG_FILE" | tail -1) || {
...
# --- Step 5: Bind extension id ---
# Bind here, immediately after registration and by the same deployer, so
# deploy -> register -> bind is one uninterrupted operator sequence. The setter is
# owner-only and verifies the registry maps EXTENSION_ID to this exact contract, so
# a pre-registration of the contract's address under another extension cannot
# capture the binding. Passing the just-registered EXTENSION_ID explicitly avoids any
# reliance on scanning for the "first" match.
step 5 "Bind extension id"
cd "$PROJECT_DIR/tools"
if ! go run ./cmd/set-extension-id -a "$ADDRESSES_FILE" -c "$CHAIN_URL" \
        --instructionSender "$INSTRUCTION_SENDER" --extension-id "$EXTENSION_ID" 2>>"$LOG_FILE"; then
    echo -e "${RED}Binding failed. Logs:${NC}" >&2
    cat "$LOG_FILE" >&2
    die "setExtensionId failed — see output above"
fi
log "Bound $INSTRUCTION_SENDER to extension $EXTENSION_ID"
