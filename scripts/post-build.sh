...
#   TEE_PROFILE         — local | testnet-sim | confidential-space (fail-fast validated;
#                         explicit testnet-sim required for MODE=1 on a public chain)
#   EXPECTED_CODE_HASH  — REQUIRED on confidential-space: measured image digest from
#                         YOUR build, cross-checked against proxy /info
#                         (see docs/production-allowlisting.md)
#   EXPECTED_PLATFORM   — REQUIRED on confidential-space: measured platform hash from
#                         YOUR build, cross-checked against proxy /info
...
# --- Resolve chain + fail-fast validate the TEE profile ---
# register-tee is about to put attestation data on-chain; the MODE /
# SIMULATED_TEE combination must be coherent with what the operator selected.
CHAIN="${CHAIN:-}"
if [[ -z "$CHAIN" ]]; then
...
# On TEE_PROFILE=confidential-space the tool refuses to trust the proxy /info
# codeHash on its own: supply the measured values of YOUR build via
# EXPECTED_CODE_HASH and EXPECTED_PLATFORM. Both are required on the
# confidential-space profile — allow-tee-version refuses to run there without
# them — and both are optional on the simulated profiles, where the /info values
# are development constants. See docs/production-allowlisting.md.
step 1 "Allow TEE version"
cd "$PROJECT_DIR/tools"

ALLOW_ARGS=()
[[ -n "${EXPECTED_CODE_HASH:-}" ]] && ALLOW_ARGS+=(-expected-codehash "$EXPECTED_CODE_HASH")
[[ -n "${EXPECTED_PLATFORM:-}" ]]  && ALLOW_ARGS+=(-expected-platform "$EXPECTED_PLATFORM")
