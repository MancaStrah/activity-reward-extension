...
#   --tunnel         require a reachable ngrok agent and wire its URL into .env
#                    (start the agent yourself) — see docs/ngrok.md
...
USE_TUNNEL=false
CHAIN=""
# CHAIN is used below to pick .env.<chain> before the file is loaded, but
# `set -a; source .env` then re-exports whatever CHAIN the file sets. Without a
# separate record of the flag, `--chain local` against a coston2 .env would
# silently run on the public testnet. Re-applied after loading.
CHAIN_FROM_FLAG=""
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --chain) [[ $# -ge 2 ]] || die "--chain requires a value (local|coston|coston2)"
                 CHAIN="$2"; CHAIN_FROM_FLAG="$2"; shift 2 ;;
        --chain=*) CHAIN="${1#--chain=}"; CHAIN_FROM_FLAG="$CHAIN"; shift ;;
